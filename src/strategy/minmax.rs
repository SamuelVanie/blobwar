...
fn minmax_iter(node: &Configuration, depth: u8, maximizing_player: bool) -> (Option<Movement>, i8) {
	...
	best = {
		...
		new_val = std::cmp::max(best.1, minmax_iter(&node.play(&child), depth - 1, maximizing_player).1);
	    }else{
		new_val = std::cmp::min(best.1, minmax_iter(&node.play(&child), depth - 1, maximizing_player).1);
	    };
...
/// Minimax algorithm using imperative coding style
fn minmax_inp(node: &Configuration, depth: u8, maximizing_player: bool) -> (Option<Movement>, i8) {
    if depth == 0 || node.movements().next().is_none() {
        return (None, node.value());
    }
    if maximizing_player {
	let best = node.movements()
	    .map(|child| (child, minmax_iter(&node.play(&child), depth - 1, maximizing_player).1))
...
fn minmax_par(node: &Configuration, depth: u8, maximizing_player: bool) -> (Option<Movement>, i8) {
    ...
    if maximizing_player {
	let (best_child, best_value) = movements.into_par_iter()
            .map(|child| (child, minmax_iter(&node.play(&child), depth - 1, maximizing_player).1))
            .max_by_key(|&(_, value)| value)
            .unwrap();
...
impl Strategy for MinMax {
	...
	minmax_par(state, self.0, state.current_player).0
    }
}
